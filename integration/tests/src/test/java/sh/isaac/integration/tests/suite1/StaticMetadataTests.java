...
public class StaticMetadataTests
{
	...
	public void testChildren()
	{
		...
		Get.assemblageService().getSemanticChronologyStream(TermAux.SEMANTIC_TYPE.getNid()).forEach(semanticC -> {
			...
			LatestVersion<SemanticVersion> latest = semanticC.getLatestVersion(StampCoordinates.getDevelopmentLatestActiveOnly());
			if (latest.isPresent())
			{
				ConceptChronology cc = Get.conceptService().getConceptChronology(latest.get().getReferencedComponentNid());
				LatestVersion<ConceptVersion> cv = cc.getLatestVersion(StampCoordinates.getDevelopmentLatestActiveOnly());
				if (cv.isPresent())
				{
