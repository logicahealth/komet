...
public class QueryProviderTest {
	...
	public void testSizeLimits2() {
		...
		int expectedMaxHits = 602;  //May need to change this when tweaking metadata... until we come up with a proper "testing" terminology to use for things like this.
	...
	public void testPredicate() {
		...
		//no predicate
		Assert.assertEquals(di.query("so*", false, null, null, null, 1, Integer.MAX_VALUE, null).size(), 466);
		
		//no fail predicate
		Assert.assertEquals(di.query("so*", false, null, (nid -> true), null, 1, Integer.MAX_VALUE, null).size(), 466);
