 ...
 */
public class ConverterOptionParam implements ConverterOption {
	 ...
	 */
	static ConverterInfo findBySrcArtifactId(String srcArtifactId) {
	 ...
	 */
	static Pair<ConverterInfo, String> findConverterTypeAndExtensionBySrcArtifactId(String srcArtifactId) {
		//Split this routine, so we do all of the exact matches first
		for (ConverterInfo sct : SupportedConverterTypes.values()) {
	...
	static ConverterInfo findByIBDFArtifactId(String ibdfArtifactId) {
	 ...
	 */
	static ConverterInfo findByMojoName(String mojoName) {
		for (SupportedConverterTypes sct : SupportedConverterTypes.values()) {
	...
	public static ConverterOptionParam[] fromArtifact(File localMavenPath, SupportedConverterTypes converter, String converterVersion, String baseMavenUrl,
		...
		try {
			ConverterOptionParam.LOG.debug("Trying to read 'options.json' for {} from '{}' '{}'", converter.getNiceName(), converterVersion, baseMavenUrl);
			...
			if (localMavenPath != null && localMavenPath.isDirectory()) {
				...
				for (File versionFolder : browseFolder.listFiles()) {
					if (versionFolder.isDirectory() && versionFolder.getName().equals(converterVersion)) {
						File zippedJsonFile = new File(versionFolder, converter.converterArtifactId + "-" + converterVersion + "." + ConverterOptionParam.MAVEN_FILE_TYPE + ".zip");
						if (zippedJsonFile.exists()) {
							try {
								ZipFile zipFile = new ZipFile(zippedJsonFile);
								zipFile.extractAll(tempFolder.getCanonicalPath());
								ConverterOptionParam.LOG.info("Read json options from local maven folder");
								return processFolderWithJson(converter, tempFolder);
							} catch (ZipException e) {
								ConverterOptionParam.LOG.error("error extracting local options zip file", e);
							}
						} else {
							ConverterOptionParam.LOG.warn("Found version folder, but no metadata?");
						}
					}
				}
			}
			...
			if (!pomFile.exists()) {
				ConverterOptionParam.LOG.debug("Throwing back an exception, as no pom was readable for the specified artifact");
			...
			try {
				final URL config = MavenArtifactUtils.makeFullURL(baseMavenUrl, mavenUsername, mavenPassword, converter.converterGroupId,
						converter.converterArtifactId, converterVersion, "", ConverterOptionParam.MAVEN_FILE_TYPE + ".zip");
				...
				// converter.
				ConverterOptionParam.LOG.info("No config file found for converter " + converter);
			...
			try {
				...
				ConverterOptionParam.LOG.error("Problem cleaning up temp folder {}", tempFolder, e);
			}
		}
	}
	 ...
	 * @throws IOException
	 */
	static ConverterOptionParam[] processFolderWithJson(SupportedConverterTypes converter, File unzippedFolder) throws IOException {
		...
		for (File jsonFile : unzippedFolder.listFiles(new FilenameFilter() {
			...
			if (jsonFile.getName().startsWith(converter.getConverterMojoName())) {
				...
				if (ConverterOptionParam.LOG.isDebugEnabled()) {
					ConverterOptionParam.LOG.debug("From file {} Found options: {}", jsonFile.getName(), Arrays.toString(temp));
				} else {
					ConverterOptionParam.LOG.info("From file {} Read {} options", jsonFile.getName(), temp.length);
				}
				break;
			}
