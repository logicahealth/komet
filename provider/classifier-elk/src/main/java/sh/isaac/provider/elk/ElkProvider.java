...
public class ElkProvider
    ...
    private void startMe() {
        LOG.info("Starting LogicProvider-ELK for change to runlevel: " + LookupService.getProceedingToRunLevel());
        classifierServiceMap.clear();
        pendingLogicTasks.clear();
        // read from disk...
        DataStore store = Get.service(DataStore.class);
        File logicProviderDir = new File(store.getDataStorePath().toAbsolutePath().toFile(), "logic-provider-elk");
    ...
    @PreDestroy
    private void stopMe() {
        LOG.info("Stopping LogicProvider-ELK for change to runlevel: " + LookupService.getProceedingToRunLevel());
    ...
    private class StopMeTask extends TimedTaskWithProgressTracker {
        ...
        protected Object call() throws Exception {
            try {
                updateMessage("Completing pending elk logic tasks");
                ...
                updateMessage("Writing elk classifier results");
                ElkProvider.this.classifierServiceMap.clear();
                ElkProvider.this.pendingLogicTasks.clear();
                ...
                // write to disk...
                try (DataOutputStream output = new DataOutputStream(new FileOutputStream(ElkProvider.this.classifierResultsFile))) {
                    buff.write(output);
                } catch (IOException e) {
                    LOG.error(e);
                }
    ...
    public ClassifierService getClassifierService(ManifoldCoordinateImmutable manifoldCoordinate) {
        ...
        if (!classifierServiceMap.containsKey(key)) {
            classifierServiceMap.putIfAbsent(key,
                    new ElkClassifier(manifoldCoordinateImmutable));
        }
