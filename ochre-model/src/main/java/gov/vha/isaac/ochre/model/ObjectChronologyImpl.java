...
public abstract class ObjectChronologyImpl<V extends ObjectVersionImpl>
	 ...
	 * data for this chronicle was read. If the write sequence does not match the
	 * write sequences in the persistence storage, the data needs to be merged
	 * prior to writing, according to the principles of a
	...
	/**
	 * Concept sequence if a concept or description, Sememe sequence if a sememe.
	 */
	...
	/**
	 * Position in the data where chronicle data ends, and toMergeVersion data
	 * starts
	 */
	...
	/**
	 * Stores the location where the chronicle data ends, and the toMergeVersion
	 * data starts.
	 *
	...
	/**
	 * Use to add a new toMergeVersion to the chronicle.
	 *
	 * @param version the toMergeVersion to add
	 */
	protected void addVersion(V version) {
		...
		if (versionListReference != null) {
			versionListReference.get().add(version);
		}
	}

	/**
	 * Get data to write to datastore, use the writeSequence as it was originally
	...
	public byte[] getDataToWrite(int writeSequence) {
		...
		if (unwrittenData == null) {
			...
			db.putInt(0); // zero length toMergeVersion record. 
			db.trimToSize();
			return db.getData();
		}
		...
		if (writtenData != null) {
			db.put(writtenData, versionStartPosition, writtenData.length - versionStartPosition - 4); // 4 for the zero length toMergeVersion at the end. 
		}
		...
		db.putInt(0); // last data is a zero length toMergeVersion record
		db.trimToSize();
		return db.getData();
	}
	...
	public byte[] mergeData(int writeSequence, byte[] dataToMerge) {
		...
		DataBuffer db = new DataBuffer(512);
		writeChronicleData(db);
		OpenIntObjectHashMap<V> versionSequenceVersionMap = new OpenIntObjectHashMap<>();
		for (V version : getVersionList()) {
			versionSequenceVersionMap.put(version.getVersionSequence(), version);
		}
		for (V toMergeVersion : getVersionList(dataToMerge)) {
			int versionSequenceToTest = toMergeVersion.getVersionSequence();
			if (versionSequenceVersionMap.containsKey(versionSequenceToTest)) {
				V thisVersion = versionSequenceVersionMap.get(versionSequenceToTest);
				// see if stamp is different...
				if (toMergeVersion.getStampSequence() != versionSequenceVersionMap.get(versionSequenceToTest).getStampSequence()) {
					// reused version sequence could be 1) a version canceled, or 2) concurrent creates...
					// tell by the author sequence. 
					if (toMergeVersion.getAuthorSequence() == thisVersion.getAuthorSequence()) {
						if (toMergeVersion.getTime() == Long.MIN_VALUE) {
							// replace with the canceled version
							versionSequenceVersionMap.put(nid, thisVersion);
						} else if (thisVersion.getTime() == Long.MIN_VALUE) {
							// leave the canceled version...
						} else {
							// Since this object is being serialized it's version takes precedence over previously written versions
						}
					}
				} else {
					// concurrent creation of versions in parallel by two authors; keep seperate
					toMergeVersion.setVersionSequence(nextVersionSequence());
					versionSequenceVersionMap.put(toMergeVersion.getVersionSequence(), toMergeVersion);
				}
			} else {
				versionSequenceVersionMap.put(toMergeVersion.getVersionSequence(), toMergeVersion);
			}
		...
		versionSequenceVersionMap.values().forEach((version) -> {
			int startWritePosition = db.getPosition();
			db.putInt(0); // placeholder for length
			version.writeVersionData(db);
			int versionLength = db.getPosition() - startWritePosition;
	...
	private List<? extends V> getVersionList(byte[] versionData) {
		if (versionStartPosition == -1) {
			// object not constructed via serialization. Must compute toMergeVersion start postion. 
		...
		DataBuffer bb = new DataBuffer(versionData);
		bb.setPosition(versionStartPosition);

		SequenceSet versionSequenceSet = new SequenceSet();
		...
		while (nextPosition < bb.getLimit()) {
			...
			if (versionLength > 0) {
				...
				V version = makeVersion(stampSequence, bb);
				if (!versionSequenceSet.contains(version.getVersionSequence())) {
					versionSequenceSet.add(version.getVersionSequence());
					results.add(version);
				}
			} else {
	...
	public List<? extends V> getVersionList() {
		...
		while (results == null) {
			...
			if (unwrittenData != null) {
				for (Map.Entry<Integer, V> entry : unwrittenData.entrySet()) {
					V version = entry.getValue();
					if (!versionSequenceSet.contains(version.getVersionSequence())) {
						versionSequenceSet.add(entry.getValue().getVersionSequence());
						results.add(version);
					}
				}
			}
	...
	/**
	 * Used to retrieve a single toMergeVersion, without creating all
	 * toMergeVersion objects and storing them in a toMergeVersion list.
	 *
	 * @param stampSequence the stamp sequence that specifies a particular
	 * toMergeVersion
	 * @return the toMergeVersion with the corresponding stamp sequence
	 */
	 ...
	 * corresponding toMergeVersion object. The subclass is not responsible to
	 * add the toMergeVersion to the toMergeVersion list, that task is performed
	 * by the calling method ({@code maveVersions}).
	 *
	 * @param stampSequence the stamp sequence for this toMergeVersion
	 * @param bb the data buffer
	 * @return the toMergeVersion object
	 */
	protected abstract V makeVersion(int stampSequence, DataBuffer bb);

	/**
	 *
	 * @return a stream of the stampSequences for each toMergeVersion of this
	 * chronology.
	 */
	@Override
