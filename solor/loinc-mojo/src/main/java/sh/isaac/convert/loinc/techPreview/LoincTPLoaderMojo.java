...
public class LoincTPLoaderMojo extends ConverterBaseMojo
{
	...
	public void execute() throws MojoExecutionException
	{
		...
		try
		{
			...
			for (final File f : this.inputFileLocation.listFiles())
			{
				...
				if (f.isFile() && f.getName().toLowerCase().endsWith(".zip"))
				{
					if (f.getName().toLowerCase().contains("technologypreview"))
					{
						if (tpZipFile != null)
						{
							throw new RuntimeException("Found multiple zip files in " + this.inputFileLocation.getAbsolutePath());
						}
						tpZipFile = f;
					}
					else
			...
			if (tpZipFile == null)
			{
				throw new RuntimeException("Couldn't find the tech preview zip file in " + this.inputFileLocation.getAbsolutePath());
			}
			loincExpressionDebug = new BufferedWriter(new FileWriter(new File(this.outputDirectory, "ExpressionDebug.log")));
			...
			this.importUtil = new IBDFCreationUtility(Optional.empty(), Optional.of(MetaData.LOINC_SOLOR_MODULES), this.outputDirectory, this.converterOutputArtifactId,
				this.converterOutputArtifactClassifier, 
				this.converterOutputArtifactVersion,
				false, releaseDate.getTime(),  
			...
			final ComponentReference metadata = ComponentReference.fromConcept(this.importUtil.createConcept("LOINC Tech Preview Metadata" + IBDFCreationUtility.metadataSemanticTag, true,
					MetaData.SOLOR_CONTENT_METADATA.getPrimordialUuid()));
			
			this.importUtil.loadTerminologyMetadataAttributes(metadata, this.converterSourceArtifactVersion, 
					Optional.of(loincData.getReleaseDate()), this.converterOutputArtifactVersion, Optional.ofNullable(this.converterOutputArtifactClassifier), this.converterVersion);
			this.importUtil.addStaticStringAnnotation(metadata, this.converterSourceLoincArtifactVersion,  MetaData.SOURCE_ARTIFACT_VERSION.getPrimordialUuid(), State.ACTIVE);
			...
			this.importUtil.loadMetaDataItems(Arrays.asList((new PropertyType[] {refsets, annotations, descTypes})), metadata.getPrimordialUuid());
			...
			while (expressionLine != null)
			{
				try
				{
					...
					{
						final String[] loincConceptData = loincNumToData.get(expressionLine[ler.getPositionForColumn("mapTarget")]);
						...
						{
							ParseTree parseTree;
							final String definitionSctid = expressionLine[ler.getPositionForColumn("definitionStatusId")];
							...
							final UUID expressionId = UUID.fromString(expressionLine[ler.getPositionForColumn("id")]);
							...
							final String loincNum = loincConceptData[loincData.getPositionForColumn("LOINC_NUM")];
							final ComponentReference concept = ComponentReference.fromConcept(this.importUtil.createConcept(buildUUID(loincNum)));
							conCounter++;
							this.importUtil.addRelationshipGraph(concept, expressionId, expression, true, null, null);
							this.importUtil.addRefsetMembership(concept, PT_Refsets.Refsets.ALL.getProperty().getUUID(), State.ACTIVE, null);
							...
							for (final String property : annotations.getPropertyNames())
							{
								...
								if (!StringUtils.isBlank(data))
								{
									this.importUtil.addStringAnnotation(concept, data, annotations.getProperty(property).getUUID(), State.ACTIVE);
								}
							}
						}
					}
