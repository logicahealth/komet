...
public abstract class AbstractUuidList extends AbstractList {
    ...
    /**
     * Inserts <code>length</code> dummy elements before the specified position into the receiver. Shifts the
     * element currently at that position (if any) and any subsequent elements to the right. <b>This method
     * must set the new size to be <code>size()+length</code>.
     *
     ...
     * <code>receiver.size()</code>, if all elements in the receiver are less than the specified key. Note that
     ...
     * otherwise continues.
     * @return <code>false</code> if the procedure stopped before all elements where iterated over, <code>true</code>
     * otherwise.
     */
     ...
     * is within bounds.</b> Precondition (unchecked): <code>index &gt;= 0 && index &lt; size()</code>.
     *
     ...
     * the right one for your data set.</b> It is generally better to call <code>sort()</code> or
     * <code>sortFromTo(...)</code> instead, because those methods automatically choose the best sorting
     * algorithm.
     *
     ...
     * e2)</code> must not throw a <code>ClassCastException</code> for any elements <code>e1</code> and <code>e2</code> in
     * the range). <p>
     *
     ...
     * shortens the list by <code>(to - from + 1)</code> elements.
     *
     ...
     * allocate new memory but simply set some instance variable like <code>size</code>.
     *
