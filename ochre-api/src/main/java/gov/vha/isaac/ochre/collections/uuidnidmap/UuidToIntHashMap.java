...
public class UuidToIntHashMap extends AbstractUuidToIntHashMap implements UuidToIntMap {
    ...
    /**
     * Removes all (key,value) associations from the receiver. Implicitly calls <code>trimToSize()</code>.
     */
    @Override
    public void clear() {
     ...
     * method before <code>put()</code>ing a large number of associations boosts performance, because the receiver
	...
	public List<UUID> keysOf(int value) {
		List<Integer> indexes = indexesOfValue(value);
                ...
                indexes.stream().map((index) -> index * 2).forEach((msb) -> {
                    int lsb = msb + 1;
