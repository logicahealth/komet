 ...
 * @author kec
 */
public class NativeImportTemplate extends TimedTaskWithProgressTracker<Integer> {

    final File importFile;

    public NativeImportTemplate(File importFile) {
    ...
    protected Integer call() throws Exception {
        try (ZipFile zipFile = new ZipFile(importFile, Charset.forName("UTF-8"))) {
            ...
            for (int i = 0; i < identifierCount; i++) {
                ...
                VersionType versionType = VersionType.getFromToken(dis.readByte()); // :MEC: type for assemblage
                // DO something with the identifier data before going to the next one.
                completedUnitOfWork(); // progress indicator
            ...
            try {
                while (true) {
                    readObject(ibdfIs); // :MEC: concepts & semantics
                }
