...
public class TimePickerContent extends VBox {
    ...
    private boolean is24HourViewWithAmPm = false;
    ...
    private ObjectProperty<TimePickerContent.TimeUnit> unit = new SimpleObjectProperty<>(TimePickerContent.TimeUnit.HOURS);
    ...
    TimePickerContent(final TimePicker jfxTimePicker) {
        ...
        unit.addListener((o, oldVal, newVal) -> {
            ...
            } else if (newVal == TimePickerContent.TimeUnit.MINUTES) {
    ...
    protected BorderPane createContentPane(LocalTime time, boolean _24HourView) {
        ...
        EventHandler<? super MouseEvent> mouseActionHandler = (event) -> {
            // TODO get updates into original source.
            if (event.getClickCount() == 2) {
                ...
                if (_24HourView) {
                    if (distance(0, 0, dx, dy) >= (contentCircleRadius - shift - (2 * selectionCircle.getRadius()))) {
                        ...
                        if (unit.get() == TimeUnit.HOURS) {
                            selectedTime.setValue(selectedTime.getValue().withHour(timeValue));
                        }
                    } else {
                        hoursContent.getChildren().get(0).setVisible(false);
                        hoursContent.getChildren().get(1).setVisible(true);
                        ...
                        if (is24HourViewWithAmPm && unit.get() == TimeUnit.HOURS) {
                            if (timeValue >= 12) {
                                period.setValue(PM);
                                selectedHourLabel.setText(Integer.toString(timeValue));
                                if (timeValue > 12) {
                    ...
                    timeValue = (index + 9) % 12 == 0 ? 12 : (index + 9) % 12;
                    if (unit.get() == TimeUnit.HOURS) {
                        if (period.getValue().equals(PM)) {
                            if (timeValue < 12) {
                                selectedTime.setValue(selectedTime.getValue().withHour(timeValue + 11));
                            } else {
                                selectedTime.setValue(selectedTime.getValue().withHour(timeValue));
                            }
                        } else {
                ...
                if (unit.get() == TimePickerContent.TimeUnit.MINUTES) {
                    timeValue = (index + 45) % 60;
                    selectedTime.setValue(selectedTime.getValue().withMinute(timeValue));
                }
                timeLabel.get().setText(unit.get() == TimePickerContent.TimeUnit.MINUTES ? unitConverter.toString(timeValue) : Integer.toString(timeValue));
                updateValue();
            }
        };
    ...
    protected StackPane createHeaderPane(LocalTime time, boolean _24HourView) {
        ...
        if (_24HourView && is24HourViewWithAmPm) {
            _24HourView = false;
        }
        ...
        selectedHourLabel.setOnMouseClicked((click) -> unit.set(TimePickerContent.TimeUnit.HOURS));
        ...
        selectedMinLabel.setOnMouseClicked((click) -> unit.set(TimePickerContent.TimeUnit.MINUTES));
    ...
    private StackPane createHoursContent(LocalTime time, boolean _24HourView) {
        ...
        // TODO get fix into JFX sources...
        if (time.getHour() < 12) {
            pointerGroup.setVisible(true);
            pointer24HourGroup.setVisible(false);
        } else {
            pointerGroup.setVisible(false);
            pointer24HourGroup.setVisible(true);
        }
        ...
        for (int i = 0; i < 12; i++) {
            ...
            setLabelTextFill(label, this.selectedTime.get().getHour() == val);
            selectedTime.addListener((observable, oldValue, newValue) -> {
                setLabelTextFill(label, newValue.getHour() == Integer.parseInt(label.getText()));
            });
        ...
        if (_24HourView) {
            ...
            for (int i = 0; i < 12; i++) {
                ...
                // init color
                setLabelTextFill(label, this.selectedTime.get().getHour() == Integer.parseInt(label.getText()));
                selectedTime.addListener((observable, oldValue, newValue) -> {
                    setLabelTextFill(label, newValue.getHour() == Integer.parseInt(label.getText()));
                });
    ...
    private void setLabelTextFill(Label label, boolean condition) {
        if (condition) {
    ...
    private StackPane createMinutesContent(LocalTime time) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            setLabelTextFill(label, this.selectedTime.get().getMinute() == Integer.parseInt(label.getText()));
            selectedTime.addListener((observable, oldValue, newValue) -> {
                setLabelTextFill(label, newValue.getMinute() == Integer.parseInt(label.getText()));
            });
    ...
    void init() {
        ...
        if(unit.get() == TimePickerContent.TimeUnit.HOURS){
    ...
    private void switchTimeUnit(TimePickerContent.TimeUnit newVal) {
        if (newVal == TimePickerContent.TimeUnit.HOURS) {
    ...
    void updateValue() {
        if (is24HourView) {
            if (is24HourViewWithAmPm) {
                if (selectedTime.get().getHour() >= 12) {
                    period.setValue(PM);
                } else {
                    period.setValue(AM);
                }
        ...
        goToTime(selectedTime.get());
        timePicker.setValue(selectedTime.get());
    }
    ...
    private void goToTime(LocalTime time) {
        if (time != null) {
            int hour = time.getHour();
            if (is24HourViewWithAmPm) {
                selectedHourLabel.setText(Integer.toString(hour % 12 == 0 ?
                        12 : hour % 12));
