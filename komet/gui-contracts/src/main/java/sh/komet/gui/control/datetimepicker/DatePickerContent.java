...
public class DatePickerContent extends VBox {
    ...
    protected PickADate datePicker;
    ...
    DatePickerContent(final PickADate datePicker) {
    ...
    private void selectDayCell(DateCell dateCell, boolean hide) {
        ...
        for (int row = 0; row < 6; row++) {
            for (int col = 0; col < daysPerWeek; col++) {
                ...
                dayCell.addEventHandler(MouseEvent.MOUSE_CLICKED, click -> {
                    ...
                    // TODO get consistent behaviour in JFX Source.
                    // Do a fork and push. Modified to have consistent behaviour with date picker.
                    if (click.getClickCount() == 2) {
                        datePicker.hide();
                    } else {
                        DateCell selectedDayCell = (DateCell) click.getSource();
                        selectDayCell(selectedDayCell, false);
