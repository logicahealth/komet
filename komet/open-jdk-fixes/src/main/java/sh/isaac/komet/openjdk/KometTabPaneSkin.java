...
public class KometTabPaneSkin extends BehaviorSkinBase<TabPane, TabPaneBehavior> {
    ...
    private static Node clone(Node n) {
        ...
        if (n instanceof MaterialDesignIconView) {
            MaterialDesignIconView view = (MaterialDesignIconView) n;
            return new MaterialDesignIconView().setStyleClass(view.getStyleClass().get(1));
        }
        if (n instanceof FontAwesomeIconView) {
            FontAwesomeIconView view = (FontAwesomeIconView) n;
            return new FontAwesomeIconView().setStyleClass(view.getStyleClass().get(1));
        }
        if (n instanceof SvgIconographic) {
            SvgIconographic view = (SvgIconographic) n;
            return new SvgIconographic().setStyleClass(view.getStyleClass().get(1));
        }
        if (n instanceof MaterialIconView) {
            MaterialIconView view = (MaterialIconView) n;
            return new MaterialIconView().setStyleClass(view.getStyleClass().get(1));
        }
        if (n instanceof EmojiOneView) {
            EmojiOneView view = (EmojiOneView) n;
            return new EmojiOneView().setStyleClass(view.getStyleClass().get(1));
        }
        if (n instanceof Icons525View) {
            Icons525View view = (Icons525View) n;
            return new Icons525View().setStyleClass(view.getStyleClass().get(1));
        }
        if (n instanceof OctIconView) {
            OctIconView view = (OctIconView) n;
            return new OctIconView().setStyleClass(view.getStyleClass().get(1));
        }
        return null;
    }
    ...
    private final TabHeaderArea tabHeaderArea;
    private final ObservableList<TabContentRegion> tabContentRegions;
    private final Rectangle clipRect;
    private final Rectangle tabHeaderAreaClipRect;
    private Tab selectedTab;
    private boolean isSelectingTab;

    public KometTabPaneSkin(TabPane tabPane) {
   ...
   private static class StyleableProperties {
        ...
        private final static CssMetaData<TabPane,TabAnimation> OPEN_TAB_ANIMATION =
                new CssMetaData<TabPane, KometTabPaneSkin.TabAnimation>("-fx-open-tab-animation",
            ...
            @Override public StyleableProperty<TabAnimation> getStyleableProperty(TabPane node) {
                KometTabPaneSkin skin = (KometTabPaneSkin) node.getSkin();
        ...
        private final static CssMetaData<TabPane,TabAnimation> CLOSE_TAB_ANIMATION =
                new CssMetaData<TabPane, KometTabPaneSkin.TabAnimation>("-fx-close-tab-animation",
                    new EnumConverter<TabAnimation>(TabAnimation.class), TabAnimation.GROW) {
...
//            prefWidth.setValue(tmpPrefWidth);
    ...
    class TabMenuItem extends RadioMenuItem {
        ...
        public TabMenuItem(final Tab tab) {
            super(tab.getText(), KometTabPaneSkin.clone(tab.getGraphic()));
            this.tab = tab;
            setDisable(tab.isDisable());
