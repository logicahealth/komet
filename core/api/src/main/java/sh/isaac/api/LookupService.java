...
public class LookupService {
    ...
    /**
     * This is automatically done when a typical ISAAC pattern is utilized. This
     * method is only exposed as public for obscure use cases where we want to
     * utilize the javaFX task API, but are not looking to start up HK2 and
     * other various ISAAC services.
     */
    ...
    /**
     * start all core isaac services in a background thread, returning
     * immediately.
     *
     * @param callWhenStartComplete (optional) - if provided, a call back will
     * be provided notifying of successfully start of ISAAC, or providing the
     * Exception, if the startup sequence failed.
     */
    ...
    /**
     * Start the Metadata services (without starting ISAAC core services),
     * blocking until started (or failed).
     */
    ...
    /**
     * Start the WorkExecutor services (without starting ISAAC core services),
     * blocking until started (or failed).
     */
    ...
    * corruption
    ...
    /**
     * Find a service by name, and automatically fall back to any service which
     * implements the contract if the named service was not available.
     *
     * @param <T> the generic type
     * @param contractOrService May not be null, and is the contract or concrete
     * implementation to get the best instance of
     * @param name May be null (to indicate any name is ok), and is the name of
    ...
    public static void setRunLevel(int targetRunLevel) {
        final int current = getService(RunLevelController.class).getCurrentRunLevel();
            int currentRunLevel = current;
        if (current > targetRunLevel) {
    ...
    //~--- get methods ---------------------------------------------------------
    /**
     * Return the highest ranked service that implements the requested contract
    ...
    /**
     * Find the best ranked service with the specified name. If no service with
     * the specified name is available, this returns null (even if there is a
     * service with another name [or no name] which would meet the contract)
     *
     * @param <T> the generic type
     * @param contractOrService May not be null, and is the contract or concrete
     * implementation to get the best instance of
    ...
    /**
     * Return true if and only if any service implements the requested contract
     * or implementation class.
     *
     * @param contractOrImpl the contract or impl
    ...
    /**
     * Return true if and only if there is a service with the specified name. If
     * no service with the specified name is available, this returns false (even
     * if there is a service with another name [or no name] which would meet the
     * contract)
     *
     * @param contractOrService May not be null, and is the contract or concrete
     * implementation to get the best instance of
