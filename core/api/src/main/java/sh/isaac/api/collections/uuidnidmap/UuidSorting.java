...
public class UuidSorting
    ...
    * {@code list.length}, if all elements in the list are less than the specified key. Note that this
    ...
    * comparator (that is, {@code c.compare(e1, e2)} must not throw a {@code ClassCastException} for any
    * elements {@code e1} and {@code e2} in the range). <p>
    *
    ...
    * @see Comparator
    */
   public static void quickSort(long[] a, int fromIndex, int toIndex, UuidComparatorBI c) {
      rangeCheck(a.length, fromIndex, toIndex);
   ...
   private static void mergeSort1(long src[], long dest[], int low, int high) {
      ...
      if (length < SMALL) {
         for (int i = low; i < high; i++) {
            for (int j = i;
                  (j > low) &&
                  ((dest[2 * j - 2] > dest[2 * j]) ||
                   ((dest[2 * j - 2] == dest[2 * j]) && (dest[2 * j - 1] > dest[2 * j + 1])));
                  j--) {
               swap(dest, j, j - 1);
            }
         }
      ...
      if ((src[2 * mid - 2] < src[2 * mid]) ||
            ((src[2 * mid - 2] == src[2 * mid]) && (src[2 * mid - 1] <= src[2 * mid + 1]))) {
   ...
   private static void mergeSort1(long src[], long dest[], int low, int high, UuidComparatorBI c) {
      ...
      if (length < SMALL) {
         for (int i = low; i < high; i++) {
            for (int j = i;
                  (j > low) && (c.compare(dest[2 * j - 2], dest[2 * j - 1], dest[j * 2], dest[j * 2 + 1]) > 0); j--) {
               swap(dest, j, j - 1);
            }
         }
