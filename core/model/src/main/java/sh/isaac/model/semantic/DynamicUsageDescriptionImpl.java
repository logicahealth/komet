...
@Service
@Singleton
public class DynamicUsageDescriptionImpl implements DynamicUsageDescription, StaticIsaacCache
{
	...
	private int refexUsageDescriptorNid;
	private String semanticUsageDescription;
	private String name;
	private DynamicColumnInfo[] refexColumnInfo;
				...
				if (semanticVersion.isPresent())
				{
					...
					if (semantic.getAssemblageNid() == DynamicConstants.get().DYNAMIC_EXTENSION_DEFINITION.getNid())
					{
						...
						try
						{
							...
							final DynamicDataType type = DynamicDataType.valueOf((String) refexDefinitionData[2].getDataObject());
							...
							if ((defaultData != null) && (type.getDynamicMemberClass() != refexDefinitionData[3].getDynamicDataType().getDynamicMemberClass()))
							{
							...
							{
								if ((refexDefinitionData[5] != null) && ((DynamicArray<DynamicString>) refexDefinitionData[5]).getDataArray().length > 0)
								{
									final DynamicArray<DynamicString> readValidators = (DynamicArray<DynamicString>) refexDefinitionData[5];

									validators = new DynamicValidatorType[readValidators.getDataArray().length];

									for (int i = 0; i < validators.length; i++)
									{
										validators[i] = DynamicValidatorType.valueOf((String) readValidators.getDataArray()[i].getDataObject());
									}
								}
								...
								if (refexDefinitionData.length > 6)
								{
									if ((refexDefinitionData[6] != null)
											&& ((DynamicArray<? extends DynamicData>) refexDefinitionData[6]).getDataArray().length > 0)
									{
										final DynamicArray<? extends DynamicData> readValidatorsData = (DynamicArray<? extends DynamicData>) refexDefinitionData[6];

										validatorsData = new DynamicData[readValidatorsData.getDataArray().length];
					...
					else if (semantic.getAssemblageNid() == DynamicConstants.get().DYNAMIC_REFERENCED_COMPONENT_RESTRICTION.getNid())
					{
						if ((refexDefinitionData == null) || (refexDefinitionData.length < 1))
						{
							throw new RuntimeException("The Assemblage concept: " + assemblageConcept
									+ " is not correctly assembled for use as an Assemblage for " + "a DynamicSemanticData Refex Type.  If it contains a "
									+ DynamicConstants.get().DYNAMIC_REFERENCED_COMPONENT_RESTRICTION.getFullyQualifiedName()
									+ " then it must contain a single column of data, of type string, parseable as a " + IsaacObjectType.class.getName());
						}

						// col 0 is Referenced component restriction information - as a string.
						try
						{
							final IsaacObjectType type = IsaacObjectType.parse(refexDefinitionData[0].getDataObject().toString(), false);
						...
						catch (final Exception e)
						{
							throw new RuntimeException(
									"The Assemblage concept: " + assemblageConcept + " is not correctly assembled for use as an Assemblage for "
						...
						if ((refexDefinitionData.length > 1) && (refexDefinitionData[1] != null))
						{
							try
							{
								final VersionType type = VersionType.parse(refexDefinitionData[1].getDataObject().toString(), false);
	...
	public static DynamicUsageDescription mockOrRead(int assemblageNid)
	{
		...
		else
		{
			return staticCache.get(assemblageNid, nidAgain -> {
					...
					if (latest.isPresent())
					{
						if (latest.get().getSemanticType() == VersionType.COMPONENT_NID)
						{
							...
							else //something with columns
							{
								...
								{
									dsud.refexColumnInfo = mockDynamicColumnFromStatic(assemblageNid,
											new UUID[] { DynamicConstants.get().DYNAMIC_DT_BYTE_ARRAY.getPrimordialUuid() },
				...
				{
					throw new RuntimeException("Can't mock or read a static semantic with no instances in the system, and no valid metadata, " + assemblageNid
							+ " " + Get.conceptDescriptionText(assemblageNid));
				}
			});
		}
	}
		...
		switch (semantic.getVersionType())
		{
			...
			case COMPONENT_NID:
				...
				dsud.refexColumnInfo = new DynamicColumnInfo[] {
						new DynamicColumnInfo(Get.identifierService().getUuidPrimordialForNid(semantic.getAssemblageNid()), 0,
								brittleColumnTitles.isPresent() ? Get.identifierService().getUuidPrimordialForNid(brittleColumnTitles.get()[0])
										: DynamicConstants.get().DYNAMIC_DT_NID.getPrimordialUuid(),
								DynamicDataType.NID, null, true, null, null) };
				break;
			...
			case LONG:
				...
				dsud.refexColumnInfo = new DynamicColumnInfo[] {
						new DynamicColumnInfo(Get.identifierService().getUuidPrimordialForNid(semantic.getAssemblageNid()), 0,
								...
								DynamicDataType.LONG, null, true, null, null) };
				break;
			...
			case DESCRIPTION:
				...
				dsud.refexColumnInfo = new DynamicColumnInfo[] {
						new DynamicColumnInfo(Get.identifierService().getUuidPrimordialForNid(semantic.getAssemblageNid()), 0,
								...
								DynamicDataType.STRING, null, true, null, null),
						new DynamicColumnInfo(Get.identifierService().getUuidPrimordialForNid(semantic.getAssemblageNid()), 1,
								brittleColumnTitles.isPresent() ? Get.identifierService().getUuidPrimordialForNid(brittleColumnTitles.get()[1])
										: DynamicConstants.get().DYNAMIC_DT_NID.getPrimordialUuid(),
								DynamicDataType.NID, null, true, null, null),
						new DynamicColumnInfo(Get.identifierService().getUuidPrimordialForNid(semantic.getAssemblageNid()), 2,
								brittleColumnTitles.isPresent() ? Get.identifierService().getUuidPrimordialForNid(brittleColumnTitles.get()[2])
										: DynamicConstants.get().DYNAMIC_DT_NID.getPrimordialUuid(),
								DynamicDataType.NID, null, true, null, null),
						new DynamicColumnInfo(Get.identifierService().getUuidPrimordialForNid(semantic.getAssemblageNid()), 3,
						...
						switch (dt)
						{
							case BOOLEAN:
								dci.add(new DynamicColumnInfo(Get.identifierService().getUuidPrimordialForNid(semantic.getAssemblageNid()), col++,
										...
										DynamicDataType.BOOLEAN, null, true, null, null));
								break;
							case FLOAT:
								dci.add(new DynamicColumnInfo(Get.identifierService().getUuidPrimordialForNid(semantic.getAssemblageNid()), col++,
										brittleColumnTitles.isPresent() ? Get.identifierService().getUuidPrimordialForNid(brittleColumnTitles.get()[col - 1])
												: DynamicConstants.get().DYNAMIC_DT_FLOAT.getPrimordialUuid(),
										DynamicDataType.FLOAT, null, true, null, null));
								break;
							case INTEGER:
								dci.add(new DynamicColumnInfo(Get.identifierService().getUuidPrimordialForNid(semantic.getAssemblageNid()), col++,
										brittleColumnTitles.isPresent() ? Get.identifierService().getUuidPrimordialForNid(brittleColumnTitles.get()[col - 1])
												: DynamicConstants.get().DYNAMIC_DT_INTEGER.getPrimordialUuid(),
										DynamicDataType.INTEGER, null, true, null, null));
								break;
							case NID:
								dci.add(new DynamicColumnInfo(Get.identifierService().getUuidPrimordialForNid(semantic.getAssemblageNid()), col++,
										brittleColumnTitles.isPresent() ? Get.identifierService().getUuidPrimordialForNid(brittleColumnTitles.get()[col - 1])
	...
	private static DynamicColumnInfo[] mockDynamicColumnFromStatic(int assemblageNid, UUID[] defaultLabels, DynamicDataType[] dataTypes)
	{
		...
		if (columnLabelConcepts.isPresent())
		{
			...
			else
			{
				...
				{
					result[col] = new DynamicColumnInfo(Get.identifierService().getUuidPrimordialForNid(assemblageNid), col,
							Get.identifierService().getUuidPrimordialForNid(columnLabelConcepts.get()[col]), dataTypes[col], null, true, null, null);
				}
			}
		}
		else
		{
			for (int col = 0; col < defaultLabels.length; col++)
			{
				result[col] = new DynamicColumnInfo(Get.identifierService().getUuidPrimordialForNid(assemblageNid), col, defaultLabels[col], dataTypes[col],
						null, true, null, null);
			}
		}
		return result;
	...
	/**
	 * An alternative method to determine if a concept defines a dynamic semantic - which will check the cache for positive hits, but does _NOT_
	 * update the cache
	...
	public static boolean isDynamicSemanticNoRead(int assemblageConceptNid)
	{
		...
		for (final SemanticChronology descriptionSemantic : assemblageConcept.getConceptDescriptionList())
		{
			final LatestVersion<Version> descriptionVersion = ((SemanticChronology) descriptionSemantic)
					.getLatestVersion(Coordinates.Filter.DevelopmentLatest());
