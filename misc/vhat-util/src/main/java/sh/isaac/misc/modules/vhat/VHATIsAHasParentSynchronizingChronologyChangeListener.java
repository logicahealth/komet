...
public class VHATIsAHasParentSynchronizingChronologyChangeListener implements ChronologyChangeListener {
   ...
   public void handleCommit(CommitRecord commitRecord) {
      ...
      LOG.trace("HandleCommit looking for - " + semanticNidsForUnhandledLogicGraphChanges.size() + " logic graphs: " + semanticNidsForUnhandledLogicGraphChanges + " and "
      ...
      for (int logicGraphNid : semanticNidsForUnhandledLogicGraphChanges.toArray(new Integer[semanticNidsForUnhandledLogicGraphChanges.size()])) {
         ...
         LOG.trace("Running VHATIsAHasParentSynchronizingChronologyChangeListener handleChange() on VHAT LOGIC_GRAPH dynamic semantic " + sc.getPrimordialUuid() + " for concept "
               ...
               public void run() {
                  ...
                  for (DynamicVersion hasParentSemantic : hasParentAssociationDynamicSemantics) {
                     ...
                     if (!parentsAccordingToNewLogicGraphVersion.contains(targetSeq)) {
                        ...
                        MutableDynamicVersion mutableVersion = hasParentSemantic.getChronology().createMutableVersion(transaction, Status.INACTIVE, manifoldCoordinate);
                        mutableVersion.setData(hasParentSemantic.getData());
                        LOG.trace("Putting association semantic " + mutableVersion.getNid() + " " + mutableVersion.getNid() + " into the ignore list prior to commit");
                        ...
                        try {
                           Get.commitService().addUncommitted(transaction, mutableVersion.getChronology()).get();
                  ...
                  if (itemsToCommit.size() > 0) {
                     try {
                        ...
                        Frills.commitCheck(transactionReference.get().commit("Retiring VHAT has_parent semantics"));
         ...
         Runnable runnable = new Runnable() {
            public void run() {
                  ...
                  if (!parentsAccordingToHasParentAssociationDynamicSemantics.contains(parentAccordingToNewLogicGraphVersion)) {
                     ...
                     if (retiredHasParentSemanticVersion.isPresent()) {
                        ...
                        MutableDynamicVersion unretiredHasParentSemanticVersion = ((SemanticChronology) (retiredHasParentSemanticVersion.get().getChronology()))
                              .createMutableVersion(getTransaction(transactionReference), Status.ACTIVE, manifoldCoordinate);
                        ...
                        try {
                           Get.commitService().addUncommitted(getTransaction(transactionReference), ((SemanticChronology) retiredHasParentSemanticVersion.get().getChronology())).get();
                        } catch (Exception e) {
                           // New version of this semantic failed to be added to commit list, so remove semantic from list so listener won't ignore it
                        ...
                        try {
                            LOG.trace("Doing commit");
                           Frills.commitCheck(getTransaction(transactionReference).commit("Unretiring VHAT has_parent association semantic (target UUID="
                        ...
                        LOG.trace("Unretired has_parent association semantic {} with target {} for concept {}", retiredHasParentSemanticVersion.get().getPrimordialUuid(),
                        ...
                        builtObjects.add(associationSemanticBuilder.build(getTransaction(transactionReference), manifoldCoordinate).getNoThrow());

                        LOG.trace("Built new has_parent association semantic with SOURCE/CHILD={} and TARGET/PARENT={}", referencedConcept.getPrimordialUuid(),
               ...
               if (builtObjects.size() > 0) {
                  try {
                      LOG.trace("Doing commit");
                     Frills.commitCheck(getTransaction(transactionReference).commit("Committing new has_parent association semantics."));
                  } catch (RuntimeException e) {
      ...
      for (int hasParentSemanticNid : semanticNidsForUnhandledHasParentAssociationChanges) {
         ...
         final ManifoldCoordinate editCoordinate = Coordinates.Manifold.DevelopmentStatedRegularNameSort();
            ...
            public void run() {
               try {
                  ...
                  LogicGraphVersionImpl newLogicGraphSemanticVersion = ((SemanticChronology) (conceptLogicGraphSemanticChronology.get()))
                        .createMutableVersion(getTransaction(transactionReference), (hasParentAssociationDynamicSemantics.size() > 0 ? Status.ACTIVE : Status.INACTIVE), editCoordinate);
                  newLogicGraphSemanticVersion.setGraphData(parentDef.getData(DataTarget.INTERNAL));
                  // This is a semantic generated by this listener, so add it to list so listener will ignore it
                  LOG.trace("Putting logic graph " + newLogicGraphSemanticVersion.getNid() + " " + newLogicGraphSemanticVersion.getNid() + " into the ignore list prior to commit");
                  ...
                  try {
                     Get.commitService().addUncommitted(getTransaction(transactionReference), conceptLogicGraphSemanticChronology.get()).get();
                  } catch (Exception e) {
                     // New version of this semantic failed to be added to commit list, so remove semantic from list so listener won't ignore it
                   ...
                   LOG.trace("Doing commit");
                  Frills.commitCheck(transactionReference.get().commit("Committing new version of logic graph semantic " + conceptLogicGraphSemanticChronology.get().getPrimordialUuid() + " with "
