...
@Service
@Singleton
public class Frills implements DynamicColumnUtility, IsaacCache
{
	 ...
	 * )
	 * 
	 * @param wc - the write coordinate / transaction.Optional
	 * @param columnName the column name
	 * @param columnDescription the column description
	 * @return the concept chronology
	 * @throws RuntimeException the runtime exception
	 */
	public static List<Chronology> buildUncommittedNewDynamicSemanticColumnInfoConcept(WriteCoordinate wc, String columnName, String columnDescription)
	{
		return Get.service(DynamicUtility.class).buildUncommittedNewDynamicSemanticColumnInfoConcept(wc, columnName, columnDescription, null);
	}
	 ...
	 * see {@link DynamicUsageDescription}
	 *
	 * @param writeCoordinate the write coord.  If this contains a transaction, this method will not commit. If it does NOT contain a transaction, or is not specified, 
	 *     then this method will commit.
	 * 
	...
	public static DynamicUsageDescription buildNewDynamicSemanticUsageDescription(WriteCoordinate writeCoordinate, String semanticFQN,
			...
			IsaacObjectType referencedComponentRestriction, VersionType referencedComponentSubRestriction)
	{
		try
		{
			boolean commit = false;
			WriteCoordinate localWriteCoordinate = ((writeCoordinate == null)
					? Get.configurationService().getUserConfiguration(Optional.empty()).getWriteCoordinate().get()
					: writeCoordinate);
			if (localWriteCoordinate.getTransaction().isEmpty()) {
				localWriteCoordinate = new WriteCoordinateImpl(Get.commitService().newTransaction("build semantic usage description"),
						localWriteCoordinate);
				commit = true;
			}
			...
			final ConceptChronology parentConcept = Get.conceptService()
					.getConceptChronology((parentConceptNid == null) ? DynamicConstants.get().DYNAMIC_ASSEMBLAGES.getNid() : parentConceptNid);
			...
			DescriptionBuilder<? extends SemanticChronology, ? extends MutableDescriptionVersion> definitionBuilder = descriptionBuilderService
					.getDescriptionBuilder(semanticPreferredTerm, builder, MetaData.REGULAR_NAME_DESCRIPTION_TYPE____SOLOR, MetaData.ENGLISH_LANGUAGE____SOLOR);
			...
			final ConceptChronology newCon = builder.buildAndWrite(localWriteCoordinate, new ArrayList<>()).getNoThrow();

			LookupService.getService(DynamicUtility.class).configureConceptAsDynamicSemantic(localWriteCoordinate, newCon.getNid(), semanticDescription,
					columns, referencedComponentRestriction, referencedComponentSubRestriction, true);
			
			if (commit) {
				commitCheck(localWriteCoordinate.getTransaction().get().commit());
			}

			return new DynamicUsageDescriptionImpl(newCon.getNid());
		}
	...
	/**
	 * Checks if the concept is specified in such a way that it defines a mapping assemblage.See
	 * {@link IsaacMappingConstants#DYNAMIC_SEMANTIC_MAPPING_SEMANTIC_TYPE}
	 * Returns cached answers (true or false)
	 *
	 * @param conceptNid the concept nid
	 * @return true, if successful
	...
	public static int createAndGetDefaultEditModule(final int module)
	{
		...
		return EDIT_MODULE_FOR_TERMINOLOGY_CACHE.get(module, moduleAgain -> {
			...
			int[] termTypeChildren = Get.taxonomyService()
					.getSnapshotNoTree(ManifoldCoordinateImmutable.makeStated(stamp, fqnCoord))
					.getTaxonomyChildConceptNids(termTypeConcept);
			...
			if (termTypeFQN.endsWith(" modules"))
			{//Common pattern
			...
			try
			{
				WriteCoordinate wc = new WriteCoordinateImpl(TermAux.USER.getNid(), TermAux.PRIMORDIAL_MODULE.getNid(), TermAux.DEVELOPMENT_PATH.getNid(), 
						Get.commitService().newTransaction("Create edit module")); 
				int nid = Get.conceptBuilderService()
						.getDefaultConceptBuilder(termTypeFQN, ConceptProxy.METADATA_SEMANTIC_TAG, defBuilder.build(),
								MetaData.SOLOR_CONCEPT_ASSEMBLAGE____SOLOR.getNid())
						.setT5UuidNested(Get.concept(module).getPrimordialUuid())
						.buildAndWrite(wc).get().getNid();
				commitCheck(wc.getTransaction().get().commit("creating new edit module for terminology type " + Get.conceptDescriptionText(termTypeConcept)));
				return nid;
			}
			catch (Exception e)
			{
	...
	/**
	 * Walk up the module tree, looking for the module concept nid directly under {@link MetaData#MODULE____SOLOR} - return it if found, otherwise,
	 * return null.
	 * 
	 * @param conceptModuleNid the module to look up
	 * @param stampFilter - optional - uses default if not provided.  If provided, and doesn't include the metadata modules, it will use a modified
	 *            stamp that includes the metadata module, since that module is required to read the module hierarchy.It also modifies the time, to always
	 *            use the latest time when evaluating the parents, because 1) the parent hierarchy of the modules shouldn't ever change from one version 
	 *            to another and 2) often times, the metadata hierarchy gets built with a timestamp that is later than the version in the content, since 
	 *            metadata is loaded at DB build time
	 */
	private static Integer findTermTypeConcept(int conceptModuleNid, StampFilter stampFilter)
	{
		...
		try
		{
			...
			int[] parents = Get.taxonomyService().getSnapshotNoTree(ManifoldCoordinateImmutable.makeStated(stampToUse, null))
			...
			for (int current : parents)
			{
				...
				else
				{
					...
					if (recursive != null)
					{//only return this one if it had a path to MODULE_SOLOR, otherwise, let the loop continue.
						return recursive;
					}
				}
			}
	...
	{
		LanguageCoordinate lc = languageCoordinate == null ? Get.configurationService().getUserConfiguration(Optional.empty()).getLanguageCoordinate()
				: languageCoordinate;
	 ...
	 * 
	 * @param stampFilter - optional Filter - pass null to use the default stamp.In either case, this only looks for an active extended type - state
	 *            is overridden.
	 ...
	 *            false to only return the extended description type if it is present and active (returns EMPTY if the extended type is missing or
	 *            inactive)
	...
	{
		Optional<SemanticChronology> descriptionExtendedTypeAnnotationSemantic = getAnnotationSemantic(descriptionId,
		...
		if (descriptionExtendedTypeAnnotationSemantic.isPresent())
		{
			...
			if (!lsv.contradictions().isEmpty())
			{
				...
				LOG.warn("Component " + descriptionId + " " + " has DYNAMIC_SEMANTIC_EXTENDED_DESCRIPTION_TYPE annotation with " + lsv.contradictions().size()
						+ " contradictions");
			}
			...
			throw new RuntimeException(
					"Failed to find UUID DynamicSemanticData type in DYNAMIC_SEMANTIC_EXTENDED_DESCRIPTION_TYPE annotation dynamic semantic");
		}
		return Optional.empty();
	}
	 ...
	 */
	public static void lookupConceptChronologyForUnknownIdentifier(final String identifier, final TaskCompleteCallback<ConceptChronology> callback, final Integer callId)
	{
	 ...
	 * @param callId - An arbitrary identifier that will be returned to the caller when this completes
	 */
	public static void lookupConceptChronology(final int nid, final TaskCompleteCallback<ConceptChronology> callback, final Integer callId)
	{
		...
		Runnable r = new Runnable()
		{
			...
			public void run()
			{
				Optional<? extends ConceptChronology> c = Get.conceptService().getOptionalConcept(nid);
				if (c.isPresent())
				{
					callback.taskComplete(c.get(), submitTime, callId);
				}
	 ...
	 */
	public static StampFilter makeStampFilterAnalogVaryingByModulesOnly(StampFilter existingStampFilter, int requiredModuleSequence,
	...
	public String[] readDynamicColumnNameDescription(UUID columnDescriptionConcept)
	{
		...
		try
		{
			final ConceptChronology cc = Get.conceptService().getConceptChronology(columnDescriptionConcept);
			...
			for (final SemanticChronology dc : cc.getConceptDescriptionList())
			{
				...
				final LatestVersion<DescriptionVersion> descriptionVersion = ((SemanticChronology) dc)
						.getLatestVersion(Get.configurationService().getUserConfiguration(Optional.empty()).getPathCoordinate().getStampFilter());
	...
	{
		LookupService.get().getAllServiceHandles(IndexQueryService.class).forEach(index -> {
			...
			index.getService().query("hi", null, null, null, 1, Long.MAX_VALUE);
		});
	}

	/**
	 * calls {@link Frills#resetStatus(WriteCoordinate, Status, Chronology, StampFilter...)} but has types specified for concepts
	 */
	private static VersionUpdatePair<ConceptVersion> resetConceptState(WriteCoordinate wc, Status status, ConceptChronology chronology, StampFilter... stampFilters) 
			throws Exception
	{
		return resetStatus(wc, status, chronology, stampFilters);
	}

	/**
	 * 
	 * Reset the state of the chronology IFF an existing version corresponding to passed edit and/or stamp coordinates either does not exist or
	 * differs in state.
	 * 
	 * This method does NOT commit.
	 * 
	 * @param writeCoordinate - where to create the new version, and transaction.
	 * @param status - state to which to set new version of chronology
	 * @param chronology - the chronology of the object that we want to create a new version of with the specified state
	 * @param readFilters - (optional) the read coordinates to read the current state from. Defaults to the system default if not provided. When more than one
	 *            is provided, it tries each in order, until is finds the first one that is present.
	 * @return - null, if no change was required, or, the mutable that will need to be committed. Also returns the latestVersion that the state was
	 *         read from for convenience.
	 * @throws Exception
	 */
	@SuppressWarnings({ "unchecked" })
	private static <T extends Version> VersionUpdatePair<T> resetStatus(WriteCoordinate wc, Status status, Chronology chronology,
			StampFilter... readFilters) throws Exception
	{
		...
		{
			versionsHolder.set((T) ((SemanticChronology) chronology).<T> createMutableVersion(toUse), (T) latestVersion.get());
		}
		else if (chronology instanceof ConceptChronology)
		{
			versionsHolder.set((T) ((ConceptChronology) chronology).createMutableVersion(toUse), (T) latestVersion.get());
		}
		else
		{
	...
	/**
	 * Reset the state of an object to the new state, copying by creating a new version of the object with the same mutable values as the existing
	 * object.
	 * 
	 ...
	 * This method does NOT commit.
	 * 
	 * @param status - The desired new state
	 * @param componentToModify - the id of the object to change the state of
	 * @param writeCoordinate - where to write the new state.
	 * @param stampFilters - (optional) the read coordinates to read the current state from. Defaults to the system default if not provided. When more than one
	 *            is provided, it tries each in order, until is finds the first one that is present.
	 * @return - empty optional, if no change, or the uncommitted chronology of the object that was changed.
	 * @throws Exception
	 */
	public static Optional<Chronology> resetStatusWithNoCommit(Status status, int componentToModify, WriteCoordinate writeCoordinate,
			StampFilter... stampFilters) throws Exception
	{
		...
		switch (type)
		{
			case CONCEPT:
			{
				...
				VersionUpdatePair<ConceptVersion> updatePair = resetConceptState(writeCoordinate, status, cc, stampFilters);
				if (updatePair != null)
				{
			...
			case SEMANTIC:
			{
				...
				switch (semantic.getVersionType())
				{
					case DESCRIPTION:
					{
						VersionUpdatePair<DescriptionVersionImpl> semanticUpdatePair = resetStatus(writeCoordinate, status, semantic, stampFilters);
					...
					case STRING:
					{
						VersionUpdatePair<StringVersionImpl> semanticUpdatePair = resetStatus(writeCoordinate, status, semantic, stampFilters);
					...
					case DYNAMIC:
					{
						VersionUpdatePair<DynamicImpl> semanticUpdatePair = resetStatus(writeCoordinate, status, semantic, stampFilters);
					...
					case COMPONENT_NID:
					{
						VersionUpdatePair<ComponentNidVersionImpl> semanticUpdatePair = resetStatus(writeCoordinate, status, semantic, stampFilters);
					...
					case LOGIC_GRAPH:
					{
						VersionUpdatePair<LogicGraphVersionImpl> semanticUpdatePair = resetStatus(writeCoordinate, status, semantic, stampFilters);
					...
					case LONG:
					{
						VersionUpdatePair<LongVersionImpl> semanticUpdatePair = resetStatus(writeCoordinate, status, semantic, stampFilters);
					...
					case MEMBER:
						VersionUpdatePair<VersionImpl> semanticUpdatePair = resetStatus(writeCoordinate, status, semantic, stampFilters);
					...
					case UNKNOWN:
					default :
						String detail = semantic.getVersionType() + " (UUID=" + semantic.getPrimordialUuid() + ", SEMANTIC NID=" + semantic.getNid()
								+ ", REF COMP NID=" + semantic.getReferencedComponentNid() + ")";
		...
		{
			LOG.debug("Built updated version of " + type + " " + componentToModify + "<" + nid + ">" + " with state changed (from " + priorState + " to "
					+ status + ")");
		}
		else
		{
	...
	{
		return version.getClass().getSimpleName() + " STAMP=" + version.getStampSequence() + "{state=" + version.getStatus() + ", time=" + version.getTime()
				+ ", author=" + version.getAuthorNid() + ", module=" + version.getModuleNid() + ", path=" + version.getPathNid() + "}";
	}
	...
	{
		TaxonomySnapshot tss = Get.taxonomyService().getSnapshotNoTree(ManifoldCoordinateImmutable.makeStated(
				(stampFilter == null ? Get.configurationService().getUserConfiguration(Optional.empty()).getPathCoordinate().getStampFilter() : stampFilter), null));
	...
	public static Optional<SemanticChronology> getAnnotationSemantic(int componentNid, int assemblageConceptId)
	{
		...
		switch (semanticSet.size())
		{
			...
			default :
				LOG.fatal("Component " + componentNid + " has " + semanticSet.size() + " annotations of type " + Get.conceptDescriptionText(assemblageConceptId)
						+ " (should only have zero or 1) - returning arbitrary result!");
				return Optional.of(semanticSet.iterator().next());
		}
	}
	 ...
	 */
	public static Optional<String> getAnnotationStringValue(int componentId, int assemblageConceptId, StampFilter stampFilter)
	{
		try
		{
			final UUID assemblageConceptUuid = Get.identifierService().getUuidPrimordialForNid(assemblageConceptId);
			...
			Get.assemblageService()
					.getSnapshot(SemanticVersion.class,
							(stampFilter == null) ? Get.configurationService().getUserConfiguration(Optional.empty()).getPathCoordinate().getStampFilter()
									: stampFilter)
					.getLatestSemanticVersionsForComponentFromAssemblage(componentNid, assemblageConceptNid).forEach(latestSemantic -> {
						...
						else if (latestSemantic.get().getChronology().getVersionType() == VersionType.COMPONENT_NID)
						{
							values.add(((ComponentNidVersionImpl) latestSemantic.get()).getComponentNid() + "");
						}
						else if (latestSemantic.get().getChronology().getVersionType() == VersionType.LONG)
						{
						...
						else if (latestSemantic.get().getChronology().getVersionType() == VersionType.DYNAMIC)
						{
							...
							if (data.length > 0)
							{
								LOG.warn("Found multiple (" + data.length + ") dynamic data fields in semantic " + latestSemantic.get().getNid()
										+ " of assemblage type " + assemblageConceptUuid + " on component "
			...
			if (values.size() > 1)
			{
				LOG.warn("Found multiple (" + values.size() + ") " + assemblageConceptUuid + " annotation semantics on component "
						+ Get.identifierService().getUuidPrimordialForNid(componentNid) + ". Using first value \"" + values.get(0) + "\".");
			}
		...
		{
			LOG.error("Unexpected error trying to find " + assemblageConceptId + " annotation semantic on component " + componentId, e);
		}
		...
		try
		{
			...
			Get.assemblageService()
					.getSnapshot(SemanticVersion.class,
							stampFilter == null ? Get.configurationService().getUserConfiguration(Optional.empty()).getPathCoordinate().getStampFilter()
									: stampFilter)
					.getLatestSemanticVersionsForComponentFromAssemblage(componentNid, MetaData.CODE____SOLOR.getNid()).forEach(latestSemantic -> {
						//expected path
						if (latestSemantic.get().getChronology().getVersionType() == VersionType.STRING)
						{
	...
	/**
	 * If the passed in value is a {@link UUID}, calls {@link ConceptService#getOptionalConcept(int)} after converting the UUID to nid. Next, if no
	 * hit, if the
	 * passed in value is parseable as a int < 0 (a nid), calls {@link ConceptService#getOptionalConcept(int)} Next, if no hit, if the passed in value
	 * is parseable
	...
	public static Optional<? extends ConceptChronology> getConceptForUnknownIdentifier(String identifier)
	{
		...
		if (SctId.isValidSctId(localIdentifier))
		{
			...
			if (si != null)
			{
				...
				List<SearchResult> result = si.query(localIdentifier + " ", true, new int[] { MetaData.SCTID____SOLOR.getNid() }, null, null, 5,
						Long.MIN_VALUE);
				if (result.size() > 0)
				{
	...
	/**
	 * Determine if a particular description semantic is flagged as preferred IN ANY LANGUAGE. Returns false if there is no acceptability semantic.
	 *
	 ...
	 */
	public static boolean isDescriptionPreferred(int descriptionSemanticNid, StampFilter stampFilter) throws RuntimeException
	{
	 ...
	 * @param activeOnly - if true, only return true if theinverse annotation is active.If false, ignore the state of
	...
	public static int getDescriptionType(int descriptionType, StampFilter stampFilter)
	{
		...
		else
		{
			return DESC_CORE_TYPE_CACHE.get(descriptionType, typeAgain -> {
				...
				//type description.
				StampFilter stampToUse = (stampFilter == null
						? Get.configurationService().getUserConfiguration(Optional.empty()).getPathCoordinate().getStampFilter()
						: stampFilter);
	 ...
	 */
	public static List<DescriptionVersion> getDescriptionsOfType(int conceptNid, ConceptSpecification descriptionType, StampFilter stampFilter)
	{
	...
	{
		return getIdInfo(id, Coordinates.Filter.DevelopmentLatest(), null);
	}
	...
	private static IdInfo getIdInfo(String id, final StampFilter stampFilter, final LanguageCoordinate languageCoordinate)
	{
		...
		StampFilter sc = stampFilter == null ? 
				Get.configurationService().getUserConfiguration(Get.configurationService().getCurrentUserId()).getManifoldCoordinate().getViewStampFilter().getStampFilter() 
				: stampFilter;

		LanguageCoordinate lc = languageCoordinate == null ? 
				Get.configurationService().getUserConfiguration(Get.configurationService().getCurrentUserId()).getLanguageCoordinate() : languageCoordinate;
		try
		{
	 ...
	 */
	public static Optional<SemanticChronology> getLogicGraphChronology(int id, boolean stated)
	{
		LOG.debug("Getting {} logic graph chronology for {}", (stated ? "stated" : "inferred"), Optional.ofNullable(Frills.getIdInfo(id)));

		final Optional<SemanticChronology> defChronologyOptional = stated ? Get.statedDefinitionChronology(id) : Get.inferredDefinitionChronology(id);
		...
		{
			LOG.debug("NO {} logic graph chronology for {}", (stated ? "stated" : "inferred"), Optional.ofNullable(Frills.getIdInfo(id)));
			return Optional.empty();
		}
	}
	 ...
	 * @return An Optional containing a LogicGraphVersion SemanticChronology
	 */
	public static Optional<SemanticChronology> getLogicGraphChronology(int id, boolean stated, StampFilter stampFilter, LanguageCoordinate languageCoordinate,
	 ...
	 * @return An Optional containing a LogicGraphVersion SemanticChronology
	 */
	public static LatestVersion<LogicGraphVersion> getLogicGraphVersion(SemanticChronology logicGraphSemanticChronology, StampFilter stampFilter)
	{
		LOG.debug("Getting logic graph semantic for {}", Optional.ofNullable(Frills.getIdInfo(logicGraphSemanticChronology.getReferencedComponentNid())));

		final LatestVersion<LogicGraphVersion> latest = ((SemanticChronology) logicGraphSemanticChronology).getLatestVersion(stampFilter);

		if (latest.isPresent())
		{
			LOG.debug("Got logic graph semantic for {}", Optional.ofNullable(Frills.getIdInfo(logicGraphSemanticChronology.getReferencedComponentNid())));
		}
		else
		{
			LOG.debug("NO logic graph semantic for {}", Optional.ofNullable(Frills.getIdInfo(logicGraphSemanticChronology.getReferencedComponentNid())));
		}
	...
	{
		return !chronology.getSemanticChronologyList().isEmpty();
	}
			...
			for (LogicNode childOfNecessarySetNode : necessarySetNode.getChildren())
			{
				if (null == childOfNecessarySetNode.getNodeSemantic())
				{
					String msg = "Logic graph for concept NID=" + logicalExpression.getConceptBeingDefinedNid()
							...
							+ "\". Expected AndNode or ConceptNode in " + logicalExpression;
					LOG.error(msg);
								...
								if (childOfAndNode.getNodeSemantic() == NodeSemantic.CONCEPT)
								{
									...
									else
									{
										...
										String msg = "Logic graph for concept NID=" + logicalExpression.getConceptBeingDefinedNid()
												...
												+ "\". Expected ConceptNodeWithNids or ConceptNodeWithUuids in " + logicalExpression;
										LOG.error(msg);
										throw new RuntimeException(msg);
									}
								}
	 ...
	 */
	public static Stream<SemanticChronology> getSemanticForComponentFromAssemblagesFilteredBySemanticType(int componentNid, Set<Integer> allowedAssemblageNids,
			Set<VersionType> typesToExclude)
	{
		...
		{
			return Arrays.stream(semanticSequences.toArray())
					.mapToObj((int semanticSequence) -> Get.assemblageService().getSemanticChronology(semanticSequence));
		}
		else
		{
	 ...
	 * @param writeCoordinate - ensure that the returned stamp coordinate includes the module and path from this write coordinate.
	 * @return a new stamp coordinate
	 */
	public static StampFilter getStampCoordinateFromEditCoordinate(final StampFilter stampFilter, WriteCoordinate writeCoordinate)
	{
		StampFilter scLocal = stampFilter == null ? Get.configurationService().getUserConfiguration(Optional.empty()).getPathCoordinate().getStampFilter()
				: stampFilter;

		final StampPosition stampPosition = StampPositionImmutable.make(scLocal.getStampPosition().getTime(), writeCoordinate.getPathNid());
		StampFilter temp = StampFilterImmutable.make(scLocal.getAllowedStates(), stampPosition, scLocal.getModuleNids(), IntLists.immutable.empty());

		if (temp.getModuleNids().size() > 0)
		{
			MutableIntSet moduleNids = IntSets.mutable.of(temp.getModuleNids().toArray());
			moduleNids.add(writeCoordinate.getModuleNid());
			temp = StampFilterImmutable.make(scLocal.getAllowedStates(), stampPosition, moduleNids.toImmutable(), IntLists.immutable.empty());
		}
	...
	public static StampFilter getStampCoordinateFromStamp(Stamp stamp, StampPrecedence precedence)
	{
		...
		final StampFilter stampCoordinate = StampFilterImmutable.make(StatusSet.of(stamp.getStatus()), stampPosition,
				IntSets.immutable.of(stamp.getModuleNid()), IntLists.immutable.empty());
	...
	public static StampFilter getStampCoordinateFromVersion(StampedVersion version, StampPrecedence precedence)
	{
		...
		final StampFilter stampCoordinate = StampFilterImmutable.make(StatusSet.of(version.getStatus()), stampPosition,
				IntSets.immutable.of(version.getModuleNid()), IntLists.immutable.empty());
	...
	public static HashSet<Integer> getTerminologyTypes(ConceptChronology oc, StampFilter stampFilter)
	{
		...
		else
		{
			oc.getVersionList().stream().filter(version -> {
				return stampFilter.getAllowedStates().contains(version.getStatus()) && (stampFilter.getModuleNids().size() == 0 ? true
						: stampFilter.getModuleNids().contains(version.getModuleNid()) && (version.getTime() <= stampFilter.getStampPosition().getTime())
		...
		switch (obj.getIsaacObjectType())
		{
			case SEMANTIC:
			{
				...
				switch (semanticChronology.getVersionType())
				{
					...
					default :
						throw new RuntimeException(
								"Semantic with NID=" + obj.getNid() + " is of unsupported SemanticType " + semanticChronology.getVersionType());
				}
			}
	...
	public static Set<Integer> getVuidSemanticNidsForVUID(long vuID)
	{
		...
		final ImmutableIntSet vhatModules = IntSets.immutable
				.of(Frills.getAllChildrenOfConcept(MetaData.VHAT_MODULES____SOLOR.getNid(), true, true, Coordinates.Filter.DevelopmentLatest()).stream()
						.mapToInt(value -> value).toArray());

		final StampFilter stampCoordinate = StampFilterImmutable.make(StatusSet.ACTIVE_ONLY, stampPosition, vhatModules, IntLists.immutable.empty());

		final Set<Integer> matchingVuidSemanticNids = new HashSet<>();

		final Predicate<Integer> filter = (Integer t) -> {
			final Optional<? extends SemanticChronology> SemanticChronologyToCheck = (Optional<? extends SemanticChronology>) Get.assemblageService()
					.getOptionalSemanticChronology(t);
	 ...
	 *
	 *         Class to contain and hide map generated by getIdInfo(). Only useful method is toString(). The returned String is not meant to be
	 *         parsed.
	 */
