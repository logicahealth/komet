...
public class VetsExporter {
   ...
   final int  vhatPropertyTypesNid  = Get.identifierService()
                                         .getNidForUuids(vhatPropertyTypesUUID);
   ...
   final int  vhatRefsetTypesNid  = Get.identifierService()
                                       .getNidForUuids(vhatRefsetTypesUUID);
   ...
   final int  codeAssemblageConceptSeq = Get.identifierService()
                                            .getConceptSequenceForUuids(codeAssemblageUUID);
   ...
   final int  vhatCodeSystemNid  = Get.identifierService()
                                      .getNidForUuids(vhatCodeSystemUUID);
   ...
   public void export(OutputStream writeTo, long startDate, long endDate, boolean fullExportMode) {
      ...
      Get.sememeService().getAssemblageTypes().forEach((assemblageSeqId) -> {
                     assemblagesMap.put(Get.conceptSpecification(assemblageSeqId)
                                           .getPrimordialUuid(),
      ...
      Terminology.CodeSystem.Version.CodedConcepts xmlCodedConcepts =
      ...
      Get.taxonomyService().getAllRelationshipOriginSequences(Get.identifierService()
            .getNidForUuids(vhatAssociationTypesUUID)).forEach((conceptId) -> {
                     ConceptChronology<? extends ConceptVersion<?>> concept = Get.conceptService()
                                                                                 .getConcept(conceptId);

                     relationshipTypes.put(concept.getPrimordialUuid(),
      ...
      // Add to map
      Get.taxonomyService().getAllRelationshipOriginSequences(Get.identifierService()
            .getNidForUuids(vhatPropertyTypesUUID)).forEach((conceptId) -> {
                     ConceptChronology<? extends ConceptVersion<?>> concept = Get.conceptService()
                                                                                 .getConcept(conceptId);
      ...
      // Add to map
      Get.taxonomyService().getAllRelationshipOriginSequences(Get.identifierService()
            .getNidForUuids(vhatDesignationTypesUUID)).forEach((conceptId) -> {
                     ConceptChronology<? extends ConceptVersion<?>> concept = Get.conceptService()
                                                                                 .getConcept(conceptId);

                     designationTypes.put(concept.getPrimordialUuid(),
                                          getPreferredNameDescriptionType(concept.getNid()));
                  });
      ...
      // Get data, Add to map
      Get.taxonomyService().getAllRelationshipOriginSequences(Get.identifierService()
            .getNidForUuids(vhatRefsetTypesUUID)).forEach((tcs) -> {
                     ConceptChronology<? extends ConceptVersion<?>> concept = Get.conceptService()
                                                                                 .getConcept(tcs);
                     ...
                     if (concept.getPrimordialUuid().equals(vhatAllConceptsUUID) ||
                        ...
                        // read VUID
                        xmlSubset.setVUID(Frills.getVuId(concept.getNid(), STAMP_COORDINATES)
                              .orElse(null));
      ...
      ConceptChronology<? extends ConceptVersion<?>> vhatConcept = Get.conceptService()
                                                                      .getConcept(vhatCodeSystemNid);

      xmlCodeSystem.setAction(ActionType.NONE);
      xmlCodeSystem.setName(getPreferredNameDescriptionType(vhatConcept.getNid()));
      xmlCodeSystem.setVUID(Frills.getVuId(vhatCodeSystemNid, null)
                                  .orElse(null));
      ...
      try {
         ...
         XMLGregorianCalendar _xmlEffDate   = DatatypeFactory.newInstance()
                                                             .newXMLGregorianCalendar(formattedDate);
         XMLGregorianCalendar _xmlRelDate   = DatatypeFactory.newInstance()
                                                             .newXMLGregorianCalendar(formattedDate);
                     ...
                     if (fullExportMode) {
                        Get.sememeService()
                           ...
                           .forEach(mappingSememe -> {
                                       Terminology.CodeSystem.Version.MapSets.MapSet xmlMapSet =
                                       ...
                                       xmlMapSet.setVUID(Frills.getVuId(concept.getNid(), STAMP_COORDINATES)
                                             .orElse(null));
                              ...
                              Optional<LatestVersion<? extends DynamicSememe>> mappingStrExtVersion =
                                 ((SememeChronology) mappingStrExt).getLatestVersion(
                                     DynamicSememe.class, STAMP_COORDINATES);

                              // TODO:DA review
                              if (mappingStrExtVersion.isPresent()) {
                                 DynamicSememeData dsd[] = mappingStrExtVersion.get()
                                                                               .value()
                                                                               .getData();
                                 ...
                                 if (dsd.length == 2) {
                                    if (dsd[0].getDataObject()
                                       ...
                                       for (DesignationType d: getDesignations(concept,
                                             startDate,
                                             endDate,
                                       ...
                                       for (PropertyType pt: readPropertyTypes(concept.getNid(),
                                             startDate,
                                             endDate,
   ...
   private PropertyType buildProperty(SememeChronology<?> sememe,
      ...
      PropertyType property = (constructor == null) ? new gov.va.med.term.vhat.xml.model.PropertyType()
            : constructor.get();
   ...
   private SubsetMembership buildSubsetMembership(SememeChronology<?> sememe, long startDate, long endDate) {
      if (sememe.getSememeType() == SememeType.DYNAMIC) {
         ...
         long vuid = Frills.getVuId(Get.identifierService()
                                       .getConceptNid(sememe.getAssemblageSequence()),
                                    STAMP_COORDINATES)
                           .orElse(0L)
                           .longValue();
   ...
   private List<Terminology.CodeSystem.Version.MapSets.MapSet.MapEntries.MapEntry> readMapEntryTypes(int componentNid,
      ...
      Get.sememeService().getSememesFromAssemblage(Get.identifierService()
                     ...
                     @SuppressWarnings({ "unchecked", "rawtypes" })
                     Optional<LatestVersion<? extends DynamicSememe>> sememeVersion =
                        ((SememeChronology) sememe).getLatestVersion(DynamicSememe.class, STAMP_COORDINATES);

                     if (sememeVersion.isPresent() && (sememeVersion.get().value().getData() !=
                         null) && (sememeVersion.get().value().getData().length > 0)) {
                        try {
                           Terminology.CodeSystem.Version.MapSets.MapSet.MapEntries.MapEntry me =
                           ...
                           me.setVUID(Frills.getVuId(sememe.getNid(), STAMP_COORDINATES)
                                            .orElse(null));
                           ...
                           boolean isActive = sememeVersion.get()
                                                           .value()
                                                           .getState() == State.ACTIVE;
                           ...
                           if (ls == null) {
                              ...
                              DynamicSememeColumnInfo[] dsci = ls.readDynamicSememeUsageDescription(sememeVersion.get()
                                                                                                                 .value()
                                                                                                                 .getAssemblageSequence())
                                                                 .getColumnInfo();
                              DynamicSememeData dsd[] = sememeVersion.get()
                                                                     .value()
                                                                     .getData();
                        ...
                        if (descriptionVersion.isPresent()) {
                           ...
                           if (d.getAction() != ActionType.ADD) {
                              ...
                              List<DescriptionSememe<?>> coll =
                                 ((SememeChronology) sememe).getVisibleOrderedVersionList(STAMP_COORDINATES);
                           ...
                           d.setVUID(Frills.getVuId(sememe.getNid(), STAMP_COORDINATES)
                                           .orElse(null));
                                 ...
                                 // skip code and vuid properties - they are handled already
                                             if ((nestedSememe.getAssemblageSequence() !=
                                                ...
                                                if (ts.wasEverKindOf(nestedSememe.getAssemblageSequence(),
                                                   ...
                                                   PropertyType property = buildProperty(nestedSememe,
                                                                                         startDate,
                                                                                         endDate,
                                                                                         null);
                                                ...
                                                // a refset that doesn't represent a mapset
                                                   ...
                                                   SubsetMembership sm = buildSubsetMembership(nestedSememe,
                                                                                               startDate,
                                                                                               endDate);
   ...
   private String getPreferredNameDescriptionType(int conceptNid) {
      ...
      Get.sememeService()
         ...
         .forEach(sememeChronology -> {
                     ...
                     if (latestVersion.isPresent() &&
                         preferredNameExtendedType.equals(Frills.getDescriptionExtendedTypeConcept(STAMP_COORDINATES,
      ...
      if (descriptions.size() == 0) {
         ...
         String description = Frills.getDescription(conceptNid,
                                                    STAMP_COORDINATES,
   ...
   private List<Terminology.CodeSystem.Version.CodedConcepts.CodedConcept.Relationships.Relationship> getRelationships(
           ...
           long startDate,
           long endDate) {
      List<Terminology.CodeSystem.Version.CodedConcepts.CodedConcept.Relationships.Relationship> relationships =
         new ArrayList<>();

      for (AssociationInstance ai: AssociationUtilities.getSourceAssociations(concept.getNid(), STAMP_COORDINATES)) {
         SememeChronology<?> sc = ai.getData()
                                    .getChronology();
               ...
               for (DynamicSememe<?> s: coll) {
                  if (s.getTime() < startDate) {
                     ...
                     if ((oldTargetCode == null) || oldTargetCode.isEmpty()) {
                        log.error("Failed to find old target code for concept " +
                                  ai.getTargetComponent().get().getPrimordialUuid());
                     }
      ...
      return Get.sememeService()
                ...
                .anyMatch(sc -> {
                             ...
                             Optional<LatestVersion<SememeVersion>> sv =
                                ((SememeChronology) sc).getLatestVersion(SememeVersion.class, STAMP_COORDINATES);
