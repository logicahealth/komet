...
public class VetsExporter {
	...
	public void export(OutputStream writeTo, long startDate, long endDate, boolean fullExportMode) {
		...
		STAMP_COORDINATES = new StampCoordinateImpl(StampPrecedence.PATH, new StampPositionImpl(endDate, MetaData.DEVELOPMENT_PATH.getNid()),
				ConceptSequenceSet.EMPTY, Status.ANY_STATE_SET);
		// Build Assemblages map
		Get.assemblageService().getAssemblageTypes().forEach((assemblageSeqId) -> {
		...
		Get.taxonomyService().getAllRelationshipOriginSequences(
				VHATConstants.VHAT_ASSOCIATION_TYPES.getNid()).forEach((conceptId) -> {
					ConceptChronology concept = Get.conceptService().getConcept(conceptId);
		...
		{
			ConceptChronology concept = Get.conceptService().getConcept(tcs);
			// Excluding these:
		...
		Get.conceptService().getConceptChronologyStream().forEach((concept) ->
		{
			if (fullExportMode) 
			{
						...
						IsaacMappingConstants.get().DYNAMIC_SEMANTIC_MAPPING_SEMEME_TYPE.getNid()).forEach(mappingSememe -> 
				{
					...
					LatestVersion<DynamicVersion> mappingSememeVersion = mappingSememe.getLatestVersion(STAMP_COORDINATES);

					if (mappingSememeVersion.isPresent()) 
					{
						// Get referenced component for the MapSet values
						ConceptChronology cc = Get.conceptService().getConceptChronology(mappingSememeVersion.get().getReferencedComponentNid());
	...
	private List<PropertyType> readPropertyTypes(int componentNid, long startDate, long endDate, Supplier<PropertyType> constructor)
	{
		...
		Get.assemblageService().getSemanticChronologyStreamForComponent(componentNid).forEach((sememe) ->
		{
			//skip code and vuid properties - they have special handling
			if (sememe.getAssemblageSequence() != MetaData.VUID.getNid() && sememe.getAssemblageSequence() != MetaData.CODE____SOLOR.getNid()
		...
		Get.assemblageService().getSemanticChronologyStreamFromAssemblage(componentNid).forEach(sememe ->
		{
			...
			if (sememeVersion.isPresent() && sememeVersion.get().getData() != null && sememeVersion.get().getData().length > 0)
			{
				try {
					...
					String code = getCodeFromNid(sememeVersion.get().getReferencedComponentNid());
					if (null == code)
					{
						code = Frills.getDescription(sememeVersion.get().getReferencedComponentNid(), null, null).orElse("");
					}
					me.setSourceCode(code);
					boolean isActive = sememeVersion.get().getStatus() == Status.ACTIVE;
					...
					{
						DynamicColumnInfo[] dsci = ls.readDynamicUsageDescription(sememeVersion.get().getAssemblageNid()).getColumnInfo();
						DynamicData dsd[] = sememeVersion.get().getData();
						...
						for (DynamicColumnInfo d : dsci)
						{
							...
							{
								if (columnUUID.equals(DynamicConstants.get().DYNAMIC_SEMEME_COLUMN_ASSOCIATION_TARGET_COMPONENT.getPrimordialUuid()))
								{
	...
	private PropertyType buildProperty(SemanticChronology semantic, long startDate, long endDate, Supplier<PropertyType> constructor)
	{
		...
		{
			@SuppressWarnings({ "unchecked", "rawtypes" })
			LatestVersion<DynamicVersion> sememeVersion = semantic.getLatestVersion(STAMP_COORDINATES);
			if (sememeVersion.isPresent() && sememeVersion.get().getData() != null && sememeVersion.get().getData().length > 0)
			{
				newValue = sememeVersion.get().getData()[0] == null ? null : sememeVersion.get().getData()[0].dataToString();
				@SuppressWarnings({ "unchecked", "rawtypes" })
				...
				isActive = sememeVersion.get().getStatus() == Status.ACTIVE;
			}
		}
		else if (semantic.getVersionType() == VersionType.STRING)
		{
			@SuppressWarnings({ "unchecked", "rawtypes" })
			LatestVersion<StringVersion> sememeVersion = semantic.getLatestVersion(STAMP_COORDINATES);
			if (sememeVersion.isPresent())
			{
				newValue = sememeVersion.get().getString();
				@SuppressWarnings({ "unchecked", "rawtypes" })
		...
		{
			log.warn("Unexpectedly passed sememe " + semantic + " when we only expected a dynamic or a string type");
			return null;
		}
	...
	private List<DesignationType> getDesignations(ConceptChronology concept, long startDate, long endDate, Supplier<DesignationType> constructor) {
		...
		Get.assemblageService().getSemanticChronologyStreamForComponent(concept.getNid()).forEach(sememe ->
		{
			if (sememe.getVersionType() == VersionType.DESCRIPTION)
			{
				...
				if (descriptionVersion.isPresent())
				{
					...
					{
						log.warn("No extended description type present on description " + sememe.getPrimordialUuid() + " " + descriptionVersion.get().getText());
					}
					...
					if (d instanceof gov.va.med.term.vhat.xml.model.Terminology.CodeSystem.Version.CodedConcepts.CodedConcept.Designations.Designation)
					{
						...
						Get.assemblageService().getSemanticChronologyStreamForComponent(sememe.getNid()).forEach((nestedSememe) -> {
							//skip code and vuid properties - they are handled already
							
							if (nestedSememe.getAssemblageSequence() != MetaData.VUID.getNid() 
									&& nestedSememe.getAssemblageSequence() != MetaData.CODE____SOLOR.getNid())
							{
		...
		for (AssociationInstance ai : AssociationUtilities.getSourceAssociations(concept.getNid(), STAMP_COORDINATES))
		{
			...
			try
			{
				...
				else if (action != ActionType.ADD)
				{
					...
					List<DynamicSememe<?>> coll = sc.getVisibleOrderedVersionList(STAMP_COORDINATES);
					Collections.reverse(coll);
	...
	private String getPreferredNameDescriptionType(int conceptNid)
	{
		...
		{
			@SuppressWarnings({ "rawtypes", "unchecked" })
			LatestVersion<DescriptionVersion> latestVersion = sememeChronology.getLatestVersion(STAMP_COORDINATES);
	...
	private boolean hasSememeModifiedInDateRange(int nid, long startDate)
	{
		...
		{
			@SuppressWarnings({ "unchecked", "rawtypes" })
			LatestVersion<SemanticVersion> sv = sc.getLatestVersion(STAMP_COORDINATES);
			if (sv.isPresent())
			{
